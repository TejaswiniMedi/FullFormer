...
class VoxelizedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            if self.mode == 'test':
                return {'inputs': np.array(input, dtype=np.float32), 'path' : path , "voxel_path" : voxel_path}
            points = []
            coords = []
            df = []
    ...
    def get_loader(self, shuffle =True):
        return torch.utils.data.DataLoader(self, batch_size=self.batch_size, num_workers=self.num_workers, shuffle=shuffle,worker_init_fn=self.worker_init_fn)
